...
use App\Filament\Resources\BenarPengkajianResource\Pages\ListBenarPengkajians;
use App\Models\BenarPengkajian;
use Carbon\Carbon;
use Filament\Widgets\Widget;

class BenarPengkajianPercentageStats extends Widget
{
    // Tentukan file view yang akan digunakan
    protected static string $view = 'filament.pages.benar-pengkajian-percentage-stats';
    ...
    public int $total = 0;
    public int $is_suhu = 0;
    public int $is_tensi = 0;
    public float $suhu_percent = 0.0;
    public float $tensi_percent = 0.0;
    ...
    protected function calculateStats(): void
    {
        ...
        $query = BenarPengkajian::query()
            ->whereYear('tanggal', $year)
            ->whereMonth('tanggal', $month);

        $this->total = $query->count();
        $this->is_suhu = (clone $query)->where('is_suhu', true)->count();
    ...
    {
        return request()->route()?->getName() === ListBenarPengkajians::getRouteName();
    }
}
